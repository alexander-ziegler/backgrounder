...
RSpec.describe Backgrounder::WAL::FileStorage do
  ...
  it 'compacts the WAL with checkpoint, keeping only in-flight jobs' do
    ...
    storage.append(Backgrounder::WAL::Entry.new(job_id: 'job1', event: 'enqueued', data: {}, state: 'queued', timestamp: Time.now.utc))
    storage.append(Backgrounder::WAL::Entry.new(job_id: 'job2', event: 'completed', data: {}, state: 'complete', timestamp: Time.now.utc))
    storage.append(Backgrounder::WAL::Entry.new(job_id: 'job3', event: 'failed', data: {}, state: 'failed', timestamp: Time.now.utc))
    storage.append(Backgrounder::WAL::Entry.new(job_id: 'job4', event: 'started', data: {}, state: 'running', timestamp: Time.now.utc))
    storage.checkpoint
    entries = storage.replay.to_a
  ...
  #   entry = Backgrounder::WAL::Entry.new(job_id: 'yaml1', event: 'enqueued', data: { 'foo' => 'bar' }, timestamp: Time.now.utc, state: 'queued')
...
RSpec.describe 'Backgrounder DSL integration' do
  ...
  it 'lists all defined jobs' do
    Backgrounder::DSL.define_job :job1 do; end
    Backgrounder::DSL.define_job :job2 do; end
  ...
  it 'can register a job with options matching Job attributes' do
    ...
    job = Backgrounder::Job.new(args: { x: 42 }, max_retries: job_def[:opts][:retry], exclusive: job_def[:opts][:exclusive])
